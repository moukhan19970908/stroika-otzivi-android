...
sealed interface DataAction {
    ...
    data class GetHint(val text: String) : DataAction
}
...
class DataViewModel @Inject constructor(
    ...
    fun onAction(action: DataAction) {
        when (action) {
            ...
            is DataAction.GetHint -> getHints(action.text)
        }
    }
    private fun getHints(text: String) {
        appRepository.addressHint(text = text) { _, result ->
            ...
            Log.d("HINT", cityList.toString())
            Log.d("HINT", state.hintList.toString())
            println(" final LL ASDRESS ${cityList.toString()}")
        }
    }
